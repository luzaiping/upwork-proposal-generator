...
const COMPETITION_KEYWORDS = [
  'many applicants', 'highly competitive', 'lots of proposals',
  'competitive rate', '100+ applicants',
];

const BUDGET_UNCLEAR_KEYWORDS = [
  'negotiable', 'to be discussed', 'tbd', 'flexible budget', 'open budget',
];
...
function detectBudgetClarity(description: string): number {
  ...
  const hasBudget = /\$[\d,]+|\d+\s*(usd|\/hr|\/hour|per hour)/i.test(description);
...
function buildReasons(
  ...
  if (analysis.matchScore >= 75) reasons.push('Strong skill match with job requirements');
  else if (analysis.matchScore >= 50) reasons.push('Partial skill match');
  else reasons.push('Low skill match');

  if (analysis.missingSkills.length === 0) reasons.push('No missing skills detected');
  else if (analysis.missingSkills.length <= 2) reasons.push(`Minor skill gaps: ${analysis.missingSkills.join(', ')}`);
  else reasons.push(`Significant skill gaps: ${analysis.missingSkills.join(', ')}`);

  if (analysis.difficulty >= 8) reasons.push('High complexity job');
  else if (analysis.difficulty <= 4) reasons.push('Low complexity, good for quick win');

  if (competitionLevel === 'high') reasons.push('High competition detected in job post');
  if (budgetClarity < 50) reasons.push('Client budget unclear');
  else if (budgetClarity === 100) reasons.push('Budget clearly stated');

  if (analysis.recommendation === 'high') reasons.push('AI recommends applying');
  else if (analysis.recommendation === 'low') reasons.push('AI advises against applying');
...
export function scoreJob(
  ...
  const budgetClarity = detectBudgetClarity(description);
  ...
  const applyConfidence = Math.round(
    Math.min(overallScore - missingPenalty + (analysis.recommendation === 'high' ? 10 : 0), 100)
  );

  const competitionPenalty = competitionLevel === 'high' ? 15 : competitionLevel === 'medium' ? 8 : 0;
  ...
  const reasons = buildReasons(analysis, competitionLevel, budgetClarity);
